...
Unit testing is the process of testing that a particular function returns expected results for different set of inputs. A unit test is supposed to
test standard inputs and corner cases. A unit test should only test the logic defined inside the tested function. Most calls to additional functions 
should be mocked, especially if they make network or database calls.

The goal of a unit test is to answer the question: if everything defined outside this function works as expected, does this function work as expected?

Integration tests aim to test a code path end-to-end. In general, only low level functions that make external calls such as HTTP requests, file *` I/O `*,
...
[![M](https://upload.wikimedia.org/wikipedia/commons/thumb/2/2f/Google_2015_logo.svg/80px-Google_2015_logo.svg.png)](https://www.google.com/search?q=unittest+and+integration+test&source=lmns&bih=714&biw=1536&hl=en&sa=X&ved=2ahUKEwiPhs6DlOr5AhUXjOAKHbbhCsoQ_AUoAHoECAEQAA)

[![M](https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Logo_of_YouTube_%282015-2017%29.svg/70px-Logo_of_YouTube_%282015-2017%29.svg.png)](https://www.youtube.com/results?search_query=unittest+and+integration+test)

- **[unittest — Unit testing framework](https://intranet.hbtn.io/rltoken/gvyg14PB6pkk6rLIlZfMOw)** 
- **[unittest.mock — mock object library](https://intranet.hbtn.io/rltoken/IRUwzh16CVuEUH2x3KgDUA)** 
- **[How to mock a readonly property with mock?](https://intranet.hbtn.io/rltoken/sQRCH33cRr2FpNeI7L828g)** 
- **[parameterized](https://intranet.hbtn.io/rltoken/D1MlxxxgWbJa6rLsJlWTpA)** 
- **[Memoization](https://intranet.hbtn.io/rltoken/oiljWJGvcWuJibuc6cX0ww)**
...
<details>
  <summary><h3>client.py :floppy_disk:</h3></summary>
...
<details>
  <summary><h3>fixtures.py :floppy_disk:</h3></summary>
...
Implement   *` TestAccessNestedMap.test_access_nested_map_exception `*  . Use the   *` assertRaises `*   context manager to test that a   *` KeyError `*   is 
...
We don’t want to make any actual external HTTP calls. Use   *` unittest.mock.patch `*   to patch  *` requests.get `*  . Make sure it returns a   *` Mock `*  object 
with a   *` json `*   method that returns   ` test_payload `   which you parametrize alongside the   *` test_url `*   that you will pass to   *` get_json `*   with 
...
```
Use  *` unittest.mock.patch `*  to mock  *` a_method `*. Test that when calling  *` a_property `*  twice, the correct result is returned but  *` a_method `*  is only 
